...
void test_gemv422_0()
{
  ...
  printf("Test in %s at %s:%d\n", __func__, __FILE__, __LINE__);
  printf("========================================\n");
  float amat[1][2][1][2] = {0}; arange(amat);
  float xvec[1][2] = {0}; fill(xvec, 1.0f);
  float yvec[1][2] = {0}; fill(yvec, 1.0f);
  float expected_yvec[1][2] = { 1, 5 };
  float alpha = 1.0;
  float beta = 0.0;
  bool trans = false;
  ...
  printf("========== result =========");
  printf("\n yvec_expected = "); printr(expected_yvec, "%.6f"); 
  printf("\n yvec_result = "); printr(yvec, "%.6f"); 
  assert_isclose(expected_yvec, yvec);
}
